...
def build_conv_layer(cfg, *args, **kwargs):
    cfg = cfg or dict(type='Conv2d')
    if cfg.get('type', 'Conv2d') != 'Conv2d':
        raise KeyError(f"Unsupported conv layer: {cfg.get('type')}")
...
class ConvModule(nn.Sequential):
    def __init__(self,
        ...
        if act_cfg is not None:
            layers['activate'] = build_activation_layer(act_cfg)
