...
from uniad.core.bbox import bbox_cxcywh_to_xyxy, bbox_xyxy_to_cxcywh
from uniad.core.builder import build_assigner, build_sampler
from uniad.core.fp16 import force_fp32, auto_fp16
from uniad.core.init import bias_init_with_prob, constant_init
from uniad.core.tensor_utils import inverse_sigmoid, multi_apply, reduce_mean
from uniad.core.transformer import build_transformer
from .seg_head_plugin import SegDETRHead, IOU
from uniad.models.losses.dice_loss import DiceLoss


def _build_mask_transformer(cfg):
    cfg = dict(cfg)
    if cfg.get('type') == 'TransformerHead':
        cfg['type'] = 'SegMaskHead'
...
class PansegformerHead(SegDETRHead):
    ...
    def __init__(
        ...
        self.loss_mask = DiceLoss(**loss_mask)
        self.things_mask_head = _build_mask_transformer(thing_transformer_head)
        self.stuff_mask_head = _build_mask_transformer(stuff_transformer_head)
        self.count = 0
