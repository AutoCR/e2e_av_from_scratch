...
from uniad.core.base import BaseModule
from einops import rearrange
from uniad.core.tensor_utils import ConfigDict, reduce_mean
...
from uniad.models.losses.occflow_loss import DiceLossWithMasks, FieryBinarySegmentationLoss
...
def _as_config_dict(cfg):
    if isinstance(cfg, dict) and not isinstance(cfg, ConfigDict):
        return ConfigDict({key: _as_config_dict(value) for key, value in cfg.items()})
    if isinstance(cfg, list):
        return [_as_config_dict(value) for value in cfg]
    if isinstance(cfg, tuple):
        return tuple(_as_config_dict(value) for value in cfg)
    return cfg
...
class OccHead(BaseModule):
    def __init__(self, 
        ...
        transformer_decoder = _as_config_dict(transformer_decoder)
        ...
        loss_dice = dict(loss_dice)
        loss_dice.pop("type", None)
        loss_mask = dict(loss_mask)
        loss_mask.pop("type", None)
        self.loss_dice = DiceLossWithMasks(**loss_dice)
        self.loss_mask = FieryBinarySegmentationLoss(**loss_mask)
    ...
    def forward_test(
        ...
        if no_query or min(self.bev_size) < 64:
