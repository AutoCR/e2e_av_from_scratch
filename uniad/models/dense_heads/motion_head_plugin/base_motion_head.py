...
from uniad.core.transformer import build_transformer_layer_sequence
...
class BaseMotionHead(nn.Module):
    ...
    def _build_loss(self, loss_traj):
        ...
        Returns:
            None
        """
        loss_traj = dict(loss_traj)
        loss_traj.pop("type", None)
        self.loss_traj = TrajLoss(**loss_traj)
    ...
    def _load_anchors(
        ...
        *,
        ...
        Returns:
            None
        """
        if dummy_motion_anchors:
            generator = torch.Generator(device="cpu")
            generator.manual_seed(int(dummy_motion_anchor_seed))
            self.kmeans_anchors = torch.randn(
                self.num_anchor_group,
                self.num_anchor,
                self.predict_steps,
                2,
                generator=generator,
            ) * float(dummy_motion_anchor_scale)
            return

        if not anchor_info_path:
            raise ValueError(
                "Motion anchor info path is required. For fake-input smoke tests, "
                "call build_uniad(dummy_motion_anchors=True) to use deterministic generated anchors."
            )

        path = Path(anchor_info_path)
        if not path.is_file():
            raise FileNotFoundError(
                f"Motion anchor info file not found: {anchor_info_path!r}. "
                "For fake-input smoke tests, call build_uniad(dummy_motion_anchors=True) "
                "to use deterministic generated anchors."
            )

        with path.open("rb") as handle:
            anchor_infos = pickle.load(handle)
        if "anchors_all" not in anchor_infos:
            raise KeyError(f"Motion anchor info file {anchor_info_path!r} is missing 'anchors_all'.")

        self.kmeans_anchors = torch.stack(
            [torch.as_tensor(a) for a in anchor_infos["anchors_all"]])  # Nc, Pc, steps, 2
        expected_shape = (self.num_anchor_group, self.num_anchor, self.predict_steps, 2)
        if tuple(self.kmeans_anchors.shape) != expected_shape:
            raise ValueError(
                f"Motion anchor shape {tuple(self.kmeans_anchors.shape)} does not match "
