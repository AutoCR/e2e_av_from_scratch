...
from uniad.models.utils.functional import bivariate_gaussian_activation
from .planning_head_plugin import CollisionNonlinearOptimizer
from uniad.models.losses.planning_loss import CollisionLoss, PlanningLoss
import numpy as np
import copy
...
class PlanningHeadSingleMode(nn.Module):
    def __init__(self,
        ...
        loss_planning = dict(loss_planning or {})
        loss_planning.pop("type", None)
        self.loss_planning = PlanningLoss(**loss_planning)
        ...
        for cfg in loss_collision:
            cfg = dict(cfg)
            cfg.pop("type", None)
            self.loss_collision.append(CollisionLoss(**cfg))
