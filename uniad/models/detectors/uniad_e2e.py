...
import torch
from uniad.core.fp16 import auto_fp16
import copy
import os
...
from ..dense_heads.planning_head import PlanningHeadSingleMode
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if seg_head:
            ...
            seg_head.pop("type", None)
            self.seg_head = PansegformerHead(**seg_head)
        if occ_head:
            occ_head = dict(occ_head)
            occ_head.pop("type", None)
            self.occ_head = OccHead(**occ_head)
        if motion_head:
            motion_head = dict(motion_head)
            motion_head.pop("type", None)
            self.motion_head = MotionHead(**motion_head)
        if planning_head:
            planning_head = dict(planning_head)
            planning_head.pop("type", None)
            self.planning_head = PlanningHeadSingleMode(**planning_head)
