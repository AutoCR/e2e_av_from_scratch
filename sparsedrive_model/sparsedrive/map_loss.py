...
from .nn_utils import weighted_loss
...
class LinesL1Loss(nn.Module):
    ...
    def forward(self,
        ...
        if self.beta > 0:
            ...
            loss = weighted_loss(loss, weight, reduction=reduction, avg_factor=avg_factor)
        
        else:
            loss = weighted_loss(torch.abs(pred - target), weight, reduction=reduction, avg_factor=avg_factor)
